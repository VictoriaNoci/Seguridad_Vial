...
for i in range(n_frames):
    ...
    f=open('../training/label_2/'+fichero,'r')  #DATASET
    a=0
    for x in f:
        a=x.split(' ')
    ...
    array_lidar=[]
    f=open('../LiDAR/Resultados_conv_bien_score/' + fichero,'r')  #BOUNDING BOXES LiDAR (2D) = Convertidos deteccion 3D Pointpillars
    a=0
    for x in f:
        a=x.split(' ')
    ...
    metricas=actualizar_metricas2D(array_dataset, array_lidar, IoU, metricas)
    #############################################################################################
